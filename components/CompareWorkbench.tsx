...
export function CompareWorkbench({ models, locale }: { models: Model[]; locale: Locale }) {
  ...
  return (
    <div className="space-y-6">
      <div className="grid gap-4 rounded-[30px] border border-white/10 bg-white/6 p-4 backdrop-blur-xl md:grid-cols-3 md:p-5">
        {[0, 1, 2].map((slot) => (
          ...
          <div className="overflow-x-auto">
            <table className="min-w-full text-left">
              <thead className="border-b border-white/10 bg-white/4 text-sm text-slate-300">
                <tr>
                  <th className="px-5 py-4 font-medium text-white">Metric</th>
                  {active.map((model) => (
                    <th key={model.slug} className="min-w-[220px] px-5 py-4">
              ...
              <tbody>
                ...
                <tr className="border-b border-white/6">
                  ...
                  {active.map((model) => <td key={model.slug} className="px-5 py-4 text-white">{model.speedCategory}</td>)}
                </tr>
                <tr className="border-b border-white/6">
                  <td className="px-5 py-4 text-sm text-slate-300">Input pricing</td>
                  {active.map((model) => <td key={model.slug} className="px-5 py-4 text-white">{model.pricing.input}</td>)}
                </tr>
                <tr className="border-b border-white/6">
                  <td className="px-5 py-4 text-sm text-slate-300">Output pricing</td>
