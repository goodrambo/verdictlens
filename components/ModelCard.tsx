...
export function ModelCard({ model, locale }: { model: Model; locale: Locale }) {
  ...
  return (
    <article className="group rounded-[28px] border border-white/10 bg-white/6 p-5 shadow-[0_20px_80px_rgba(0,0,0,0.18)] backdrop-blur-xl transition hover:-translate-y-1 hover:border-cyan-300/30 hover:bg-white/8">
      <div className="flex items-start justify-between gap-4">
        <div>
          <p className="text-xs uppercase tracking-[0.28em] text-slate-400">{model.provider}</p>
      ...
      <div className="mt-5 grid grid-cols-2 gap-3 text-sm">
        <div className="rounded-2xl border border-white/8 bg-slate-950/50 p-3">
          <div className="text-slate-400">{copy.labels.contextWindow}</div>
          <div className="mt-1 font-medium text-white">{model.contextWindow}</div>
        </div>
        <div className="rounded-2xl border border-white/8 bg-slate-950/50 p-3">
          <div className="text-slate-400">{copy.labels.speed}</div>
          <div className="mt-1 font-medium text-white">{model.speedCategory}</div>
        </div>
      </div>
